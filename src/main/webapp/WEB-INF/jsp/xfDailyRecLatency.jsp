...
<%
    ...
    if(exportToExcel != null && exportToExcel.toString().equalsIgnoreCase("YES")){
        ...
        response.setHeader("Content-Disposition","inline; filename=" + "dailyRecLatency.xls");
    }
%>
...
<div class="container">
    ...
    <div class="row" style="margin-bottom: 10px">
        ...
        <div style="float: right">
            <label style="padding-right: 10px">
                <button class="btn btn-default" id="button_date" onclick="displayLatency()">查询</button>
            </label>
        </div>
    </div>
    ...
    <div class="row" id="table_data">
        <table class="table table-striped" cellpadding="2px" cellspacing="2px">
            ...
            <tbody>
            <tr>
                <th>明细数据</th><th></th><th></th><th></th><th></th><th></th><th></th><th></th>
            </tr>
            </tbody>
            <tbody>
            <tr>
            ...
            <tbody>
            <c:forEach items="${xfRecLatencyList}" var="xfLatency" varStatus="loop">
                <tr>
                    <td>${loop.count}</td>
                    <td>${xfLatency.date}</td>
                    <td><a onclick="displayLatencyDetail('${xfLatency.date}');">${xfLatency.member_count}</a> </td>
                    <td>${xfLatency.avg}</td>
                    <td>${xfLatency.max}</td>
...
<script type="text/javascript">
    ...
    function displayLatency() {
        ...
        var type = "date";
        if(begin == end) type = "hour";
        window.location.href = '<%=basePath%>stat/xfDailyRecLatency?member_type='+member_type+'&version='+version+'&start_date='+begin+'&end_date='+end+'&type='+type;
    }

    function displayLatencyDetail(date) {
        var member_type = document.getElementById("userTypeSelect").value;
        var begin = document.getElementById("start_datepicker").value;
        var end = document.getElementById("end_datepicker").value;
        var version = document.getElementById("versionSelect").value;
        var type = "date";
        if(begin == end) type = "hour";
        window.location.href = '<%=basePath%>stat/xfRecLatency?member_type='+member_type+'&version='+version+'&date='+date+'&type='+type;
    }

    function TableToExcel() {
        window.location.href = '<%=basePath%>stat/xfDailyRecLatency?<%=url%>&exportToExcel=YES';
    }

    function chart(index) {
        // 基于准备好的dom，初始化echarts实例
        var title_list = ['用户总数', '均值' ,'最大值','上四分位','下四分位','最小值'];
        var title_name = title_list[index];
        var label_list = ['人数', '数量', '数量', '数量', '数量', '数量'];
